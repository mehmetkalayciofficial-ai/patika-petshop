    ...
    <AnimatePresence>
      {gorunur && (
        <motion.div
          ...
          className="fixed inset-x-0 bottom-0 z-[55] px-4 pb-[calc(env(safe-area-inset-bottom)+4.25rem)] sm:pb-5"
        >
          <motion.button
            whileTap={{ scale: 0.985 }}
            onClick={onOpen}
            className="mx-auto flex h-14 w-full max-w-md items-center gap-3 rounded-full bg-brand-500 pl-5 pr-4 text-ink-900 shadow-[0_8px_28px_rgba(240,180,41,.45)] transition hover:bg-brand-400"
          >
